 ...
 * <p>
 * A graph screen that shows glucose values for special period time.
 */
public class BloodZoomActivity extends AppCompatActivity {
    ...
    // X&Y arrays
    private ArrayList<Double> seriesList = DiaryFragment.AddGlucoseMeasurment.getSeries();
    private ArrayList<Date> domainList = DiaryFragment.AddGlucoseMeasurment.getDomainLabels();
    private ArrayList<Double> goalList = DiaryFragment.AddGlucoseMeasurment.getGoalList();
    private ArrayList<Double> newSeriesList = new ArrayList<>();
    private ArrayList<Double> newGoalList = new ArrayList<>();
    ...
    private void initUI() {
        //Toolbar settings
        Toolbar toolbar = findViewById(R.id.toolbar_blood);
        ...
        //Plot initialisation
        plotZoom = findViewById(R.id.plotZoomBlood);
        Spinner spinnerPeriod = findViewById(R.id.spinner_period_blood);
                    ...
                    case DAY:
                        ...
                        for (int i = 0; i < domainList.size(); i++) {
                            if (isToday(domainList.get(i)) == 0) {
                                ...
                                newGoalList.add(goalList.get(i));
                            }
                        }
    ...
    private void setBoundaries(ArrayList<Date> list, ArrayList<Date> domainList) {
        if (list.size() < 1) {
            ...
            plotZoom.setRangeBoundaries(85, 125, BoundaryMode.FIXED);
            ...
            plotZoom.redraw();
        } else if (list.size() > 1 && list.size() < DiaryFragment.getDomainBoundarySize()) {
            plotZoom.setRangeBoundaries(85, 125, BoundaryMode.AUTO);
            plotZoom.setRangeStep(StepMode.SUBDIVIDE, 5);
            ...
            plotZoom.setRangeBoundaries(75, 125, BoundaryMode.FIXED);
            plotZoom.setRangeStep(StepMode.SUBDIVIDE, 5);
            plotZoom.setDomainBoundaries(domainList.size() - list.size(), BoundaryMode.FIXED, domainList.size(), BoundaryMode.AUTO);
            plotZoom.setDomainStep(StepMode.SUBDIVIDE, list.size());
            plotZoom.redraw();
    ...
    private void setGraph() {
        ...
        plotZoom.getGraph().getLineLabelStyle(XYGraphWidget.Edge.BOTTOM).setFormat(new Format() {
            ...
            public StringBuffer format(Object obj, @NonNull StringBuffer toAppendTo, @NonNull FieldPosition pos) {
                ...
                return dateFormat.format(DiaryFragment.AddGlucoseMeasurment.getDomainLabels().get(i), toAppendTo, pos);
            }
    ...
    public void onClickShare(View v) {
        ...
        sharingIntent.putExtra(android.content.Intent.EXTRA_TEXT, "My blood");
